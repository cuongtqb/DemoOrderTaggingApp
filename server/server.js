...
async function createAppServer() {
  ...
  const urlMongo = `mongodb+srv://admin:admin123@cluster0.7udjv.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
  ...
  router.put("/setting", async (ctx) => {
    try {
      ...
      if (!_id) {
        ...
        setting = await Setting.updateOne(
          { _id },
          { $set: { totalPrice, tag } },
          { upsert: true }
        );
  ...
  router.get("/search", async (ctx) => {
    try {
      const getSession = await Session.findOne({ shop: process.env.SHOP });
      const client = new Shopify.Clients.Graphql(
        getSession.shop,
        getSession.accessToken
      );
      ...
      const variables = { numOrdersFirst: parseInt(numOrdersFirst) };
      if (cursorAfter) {
        variables.cursorAfter = cursorAfter;
        variables.numOrdersFirst = parseInt(numOrdersFirst);
      }
      if (cursorBefore) {
        delete variables.numOrdersFirst;
        variables.cursorBefore = cursorBefore;
        variables.numOrdersLast = parseInt(numOrdersLast);
      }
      ...
      const result = await client.query({
        data: {
          query: `
          query ($numOrdersLast: Int, $cursorBefore: String,$input: String, $cursorAfter: String, $numOrdersFirst: Int){
            orders(last: $numOrdersLast, before: $cursorBefore,query: $input, first: $numOrdersFirst, after: $cursorAfter) {
              pageInfo {
                hasNextPage
                hasPreviousPage
              }
      ...
      console.log("result errors", JSON.stringify(result.body.errors));
      if (result.body?.data) {
        ctx.status = 200;
        ctx.response.body = result.body.data;
      }
    } catch (error) {
